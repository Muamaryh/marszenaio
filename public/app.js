...
async function loadSources() {
  try {
    ...
    // Fallback default list (12 Official Anichin Providers)
    appState.sources = [
      { key: 'dramabox', name: 'DramaBox', badge: 'Popular', desc: 'Provider drama box nomor 1 di Asia' },
      { key: 'reelshort', name: 'ReelShort', badge: 'Hot', desc: 'Drama pendek romantis dan billionaire viral' },
...
function setupVideoPlayer(data, sessionId, startTime = 0) {
  ...
  if (isM3u8 && window.Hls && Hls.isSupported()) {
    ...
    hls.on(Hls.Events.AUDIO_TRACKS_UPDATED, (event, audioData) => {
      if (sessionId !== playbackSessionId) return;
      const tracks = audioData.audioTracks || hls.audioTracks || [];
      setupHlsAudioTracks(tracks);
      if (tracks.length > 0) {
    ...
    hls.on(Hls.Events.AUDIO_TRACK_SWITCHED, (event, data) => {
      ...
      if (select && typeof data.id === 'number') select.value = data.id;
    });
